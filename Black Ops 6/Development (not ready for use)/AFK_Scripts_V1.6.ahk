...
F5::  ; Start AFK Script
    ...
    GuiControlGet, Script5Radio, , Script5Radio
    if (Script6Radio) {
        script1Active := false
        script2Active := false
        script3Active := false
    ...
    GuiControlGet, Script6Radio, , Script6Radio
    if (Script6Radio) {
        script1Active := false
        script2Active := false
        script3Active := false
...
RunScript6:
    if (!scriptRunning) {
        ...
        Loop, 13 {  ; Repeat the following actions 13 times before stopping
            Send, {W Down}
            Sleep, 50
            Send, {v Down}
            Sleep, 50
        ...
        scriptRunning := false
        SetTimer, RunScript6, Off  ; Turn off the timer for RunScript4
...
F6::
    ...
    if (!scriptRunning) {
        ...
        SetTimer, RunScript5, Off  ; Turn off the timer for RunScript4
        SetTimer, RunScript6, Off  ; Turn off the timer for RunScript4
    }
return