...
; Calculate Height (MaxHeight adjusted to 80% of screen height)
MaxHeight := A_ScreenHeight * 0.80  ; Adjusted to 80% of screen height
MinHeight := 300
H := MinHeight

; Initial height calculation for content
GuiControlGet, FooterPos, Pos, Footer
H := FooterPosY + 100  ; Footer's position
H := (H > MaxHeight) ? MaxHeight : H  ; Limit height to MaxHeight (80% of screen height)

; Adjust the height of the GUI window
Gui, Show, % "w" Width " NA" " x" (A_ScreenWidth - Width) " y550 h" H
...
Gui, Add, GroupBox, % "x7 y" yPos " w" ButtonWidth " h100", AFK - Liberty Falls
...
Gui, Add, GroupBox, % "x7 y" yPos " w" ButtonWidth " h100", AFK - Terminus
...
Gui, Add, GroupBox, % "x7 y" yPos " w" ButtonWidth " h120", AFK - Citadelle Des Morts
...
Gui, Add, GroupBox, % "x7 y" yPos " w" ButtonWidth " h120", AFK - The Tombs
...
Gui, Add, CheckBox, % "x7 y" (yPos + 120) " w" ButtonWidth "r1 +0x4000 vZombiesLifeInsurance", Zombies Life Insurance

Gui, Add, Button, % "x7 y" (yPos + 150) " w" CloseButtonWidth "r1 +0x4000 gClose", Close Script
    ...
    if (!scriptRunning) {
        ...
        Loop {
            ...
            if (ErrorLevel = 0) {
                ...
                ; Break the loop to stop AFK script
                scriptRunning := false
                SetTimer, RunScript4, Off  ; Turn off the script timer
                Break
            }
            else {
	        Send, {e down}
