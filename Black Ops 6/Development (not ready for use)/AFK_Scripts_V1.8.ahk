...
LoadState() {
    ...
    if (lifeInsuranceChecked == 1) {
        GuiControl,, ZombiesLifeInsurance, 1
    } else {
...
Gui, Add, CheckBox, % "x7 y" (yPos + 130) " w" ButtonWidth "r1 +0x4000 vZombiesLifeInsurance Checked", Zombies Life Insurance
...
F5::
    ...
    SetTimer, RunScript5, Off
    SetTimer, RunScript6, Off
    scriptRunning := false  
...
RunScript6:
    ...
    ; Ensure everything is completely reset after execution
    SetTimer, RunScript6, Off
    scriptRunning := false  
    selectedScript := ""  ; Clear the selected script
